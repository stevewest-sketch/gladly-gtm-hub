    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="text-white py-[50px] px-10" style={{background: 'linear-gradient(135deg, #6B46C1 0%, #8C69F0 50%, #A78BFA 100%)'}}>
        <div className="max-w-[1200px] mx-auto text-center">
          <h1 className="text-[48px] font-bold mb-4">Content Library</h1>
      ...
      <div className="max-w-6xl mx-auto px-5 py-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {regularItems.map(item => (
              <a
                key={item._id}
                href={item.link}
                className="block bg-white border-2 border-gray-200 rounded-lg p-6 hover:border-[#8C69F0] hover:shadow-xl hover:-translate-y-1 transition-all"
              >
                {item.icon && (
                ...
                <span className="text-[#8C69F0] font-semibold text-sm">
                  View Content →
                </span>
              </a>
            ))}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
              {compactItems.map(item => (
                <a
                  key={item._id}
                  href={item.link}
                  className="block bg-white border-2 border-gray-200 rounded-lg p-5 text-center hover:border-[#8C69F0] hover:shadow-lg hover:-translate-y-1 transition-all"
                >
                  {item.icon && (
                  ...
                  <span className="text-[#8C69F0] font-semibold text-xs">
                    View →
                  </span>
                </a>
              ))}
        ...
        {/* CTA Section */}
        <div className="bg-[#C5B3F8] rounded-lg p-12 text-center mt-12">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <a
              href="#"
              className="inline-block bg-[#8C69F0] text-white px-8 py-3 rounded-md font-semibold hover:bg-[#7557d9] transition-colors"
            >
              Request Content
            </a>
            <a
              href="#"
              className="inline-block bg-white text-[#8C69F0] px-8 py-3 rounded-md font-semibold border-2 border-[#8C69F0] hover:bg-[#8C69F0] hover:text-white transition-colors"
            >
              Browse All Files
            </a>
          </div>
