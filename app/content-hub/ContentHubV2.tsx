...
import styles from './ContentHubV2.module.css';

interface ContentHubV2Props {
  entries: CatalogEntry[];
  collections: Collection[];
}

export function ContentHubV2({ entries, collections }: ContentHubV2Props) {
  ...
  const tabs = useMemo(() => {
    const featuredTab = { id: 'featured', label: 'Essentials', icon: '🔥', subsections: [] };
    ...
    const browseAllTab = { id: 'browse-all', label: 'Browse All', icon: '🗂️', subsections: [] };
  ...
  const filterEntriesBySubsection = (
    ...
    let filtered = entries.filter(e => {
      const assignment = e.contentHubCollections?.find(a =>
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter(e =>
      e.contentHubCollections?.some(assignment =>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className={styles.header}>
        <div className={styles.headerContent}>
          ...
          <p>Find it fast, use it now. Decks, one-pagers, battlecards, and tools for every customer conversation.</p>
      ...
      <main className={styles.mainContent}>
        ...
        {activeTab === 'featured' && (
          <div className={`${styles.tabPanel} ${styles.active}`}>
            ...
            {/* Your Go-To Assets Section */}
            {regularFeaturedEntries.filter(e => e.displayPriority === 'featured' || e.featured).length > 0 && (
              <div className={styles.section}>
                <div className={styles.sectionHeader}>
                  <h2 className={styles.sectionTitle}>
                    <span>⭐</span> Your Go-To Assets
                  </h2>
                  <span className={styles.sectionCount}>
                    {regularFeaturedEntries.filter(e => e.displayPriority === 'featured' || e.featured).length} resources
                  </span>
                </div>
                <div className={styles.cardGrid}>
                  {regularFeaturedEntries
                    .filter(e => e.displayPriority === 'featured' || e.featured)
                    .slice(0, 6)
                    .map(entry => (
              ...
              return recentEntries.length > 0 ? (
                <div className={styles.section}>
                  <div className={styles.sectionHeader}>
                    ...
                    <span className={styles.sectionCount}>{recentEntries.length} resources</span>
                  </div>
                  <div className={styles.cardGrid}>
            ...
            {viewMode === 'list' && (
              <div className={styles.browseList}>
                {filteredEntries.map(entry => {
                  ...
                  const icon = '📚'; // Default icon for list view
