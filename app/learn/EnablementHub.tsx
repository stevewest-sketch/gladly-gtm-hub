...
const PRODUCT_FILTERS = [
  { id: 'gladly', label: 'Gladly', color: 'bg-amber-500' },
  { id: 'gladly-team', label: 'Gladly Team', color: 'bg-blue-500' },
...
const TYPE_ICON_BG: Record<string, string> = {
  'live-replay': 'bg-red-50',
  'on-demand': 'bg-blue-50',
  course: 'bg-amber-50',
};
...
export default function EnablementHub({
  ...
  const filteredEntries = useMemo(() => {
    ...
    return result;
  }, [entries, activeType, activeRole, activeProductFilters, activeFormat, coursesOnly, searchQuery, sortBy]);
  ...
  const renderDiscoveryItem = (entry: CatalogEntry, showViews = false) => {
    ...
    return (
      <Link
        ...
        className="flex items-center gap-3 py-3 border-b border-gray-100 last:border-b-0 cursor-pointer transition-all hover:bg-gray-50 hover:-mx-3 hover:px-3 hover:rounded-md no-underline"
      >
        <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg flex-shrink-0 ${TYPE_ICON_BG[format] || 'bg-gray-100'}`}>
          {TYPE_ICONS[format] || '📺'}
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium text-gray-900 mb-0.5 truncate">{entry.title}</div>
          <div className="text-xs text-gray-400 flex items-center gap-1.5">
            <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded uppercase ${badgeColors.bg} ${badgeColors.text}`}>
              {getCategoryLabel(category)}
            </span>
            <span className="w-1 h-1 bg-gray-300 rounded-full" />
            <span>{showViews ? `${entry.viewCount || 0} views` : formatDate(entry.publishDate)}</span>
          </div>
        </div>
      </Link>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FC]">
      ...
      <main className="max-w-[1400px] mx-auto px-8 py-8">
        {/* "All Training" Tab - Full Catalog with Discovery */}
        {activeTab === 'all' ? (
          <>
            {/* Discovery Section */}
            <section className="bg-gradient-to-br from-emerald-50 via-white to-teal-50 rounded-2xl p-8 mb-10 border border-emerald-100/50">
              <div className="grid grid-cols-2 gap-12">
                ...
                <div>
                  ...
                  <div className="flex flex-col bg-white/70 rounded-xl p-3 backdrop-blur-sm">
                    {recentlyAdded.map((entry) => renderDiscoveryItem(entry, false))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-col bg-white/70 rounded-xl p-3 backdrop-blur-sm">
                    {mostPopular.map((entry) => renderDiscoveryItem(entry, true))}
                  </div>
                </div>
              </div>
            </section>
            ...
            {/* Search */}
            <div className="mb-6">
              <div className="flex items-center gap-2 px-4 py-3 bg-white border border-gray-200 rounded-lg max-w-md focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-100 transition-all">
            ...
            {/* Results Header */}
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm text-gray-500">{filteredEntries.length} training sessions</span>
